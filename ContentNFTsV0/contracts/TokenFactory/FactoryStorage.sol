...
contract FactoryStorage {
    ...
    event contractCreated(address creatorAddress, address contractAddress, uint contractID);
    ...
    mapping(uint => contractCreation) contractID;
    ...
    // Stores information about contract creation event
     function _storeInfo(address newContract, uint32 contractType) internal {
        ...
        contractCreation memory newContractCreation = 
            contractCreation(
                contractIDCounter,
                newContract,
                msg.sender,
    ...
    function updateOwner(
        address _newOwner, 
        address _contract) external onlyContract(_contract) returns (bool) {
