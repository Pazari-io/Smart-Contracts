...
contract PaymentRouter is Context, ReentrancyGuard {
     ...
     * *not* revert. Instead, it should skip over them and keep going. This is to avoid having items
     * on the marketplace that cannot be sold in the event a bugged front-end ui element is submitting 
     * bad data and causing _pushTokens() to revert during transfer. If a transfer fails, then the payment 
     * for that recipient simply won't transfer, a transferFailed() event will fire, and the loop will 
     * continue to the next recipient. We will reimburse creators for their lost earnings from the treasury 
     * in the event of a front-end glitch that causes a pushTokens() transfer to fail, and we should be able to
     * use the transferFailed() event to automate it.
     *
     */
     ...
     *
     * note This function automatically pushes tokens to the treasury contract, so our 3% never needs to be
     * collected from escrow by the treasury contract. It's a little bit of extra gas on the user that will
     * save us a lot of headache.
     */
    function _holdTokens(bytes32 _routeID, address _tokenAddress, uint256 _amount) internal nonReentrant() returns (bool) {
        // Calculate treasury's commission from _amount
        uint256 treasuryCommission = _amount * 300 / 10000;
     ...
     *
     * note The reason I chose to use bytes32 hashes instead of uints is to hopefully avoid the possibility
     * of a front-end glitch sending a buyer's money down the wrong payment route. Using hashes also obscures
     * the order in which payment routes are created, and makes it almost impossible to tell which ones were
     * created first. It also makes it impossible for other people to view each other's payment routes by 
     * guessing numbers. To find a payment route ID, you must know the route's creator, the recipients, and
     * the commissions of each recipient--in the exact order they were listed in at the time of creation.
     */
