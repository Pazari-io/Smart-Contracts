...
contract MarketplaceV0 is ReentrancyGuard {
    ...
    function createMarketItem(
        ...
        emit MarketItemCreated(itemID, nftContract, tokenID, msg.sender, amount, price);
    }
    ...
    function buyMarketItem(uint256 _itemID, uint256 _amount) external payable nonReentrant {
        ...
        uint256 tokenID = idToMarketItem[_itemID].itemID;
        ...
        if (_amount == amount) {
            // If buy order clears all available stock, then:
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        uint256 unsoldItemCount = _itemIDs.current() - _itemsSoldOut.current();
        ...
        for (uint256 i = 1; i <= itemCount; i++) {
            if (idToMarketItem[i].amount > 0) {
                // All unsold items have amount > 0
