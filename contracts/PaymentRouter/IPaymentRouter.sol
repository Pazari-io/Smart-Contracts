...
interface IPaymentRouter {
   ...
   */
  function getTaxBounds() external view returns (uint minTax, uint maxTax);
   ...
   */
  function setTreasuryAddress(address _newTreasuryAddress) external returns (bool success, address oldAddress, address newAddress);  
...
interface IAccessControlPR {
  ...
  event AdminRemoved(bytes32 routeID, address removedAdmin, address callerAdmin, string memo, uint256 timestamp);
  ...
  function addRouteAdmin(bytes32 _routeID, address _newAdmin, string memory memo) external returns (bool success);

  // Removes an address from isRouteAdmin mapping
  function removeRouteAdmin(bytes32 _routeID, address _oldAddress, string memory memo) external returns (bool success);
