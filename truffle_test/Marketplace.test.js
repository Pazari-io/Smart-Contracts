...
contract("Marketplace", (accounts) => {
  ...
  describe("test createMarketItem", async () => {
    ...
    it("can create a basic item", async () => {
      ...
      await marketplace.createMarketItem(nftContract, tokenID, price, amount, { from: accounts[0] });
      ...
      assert.equal(item.nftContract, nftContract, "Item should be created successfully nftContract");
    ...
    it("cannot create free items", async () => {
      let nftContract = token.address;
      await truffleAssert.reverts(marketplace.createMarketItem(nftContract, tokenID, 0, amount, { from: accounts[0] }));
    });

    it("cannot create item for non existing token", async () => {
      let nftContract = token.address;
      await truffleAssert.reverts(marketplace.createMarketItem(nftContract, 123, 0, amount, { from: accounts[0] }));
    });

    it("cannot create items when not enough tokens", async () => {
      let nftContract = token.address;
      await truffleAssert.reverts(
        marketplace.createMarketItem(nftContract, tokenID, price, amount + 1, { from: accounts[0] }),
      );
    });
  });
  ...
  describe("test buyMarketItem", async () => {
    ...
    it("can buy a basic item", async () => {
      ...
      await marketplace.buyMarketItem(itemID, amountSell, { value: price * amountSell, from: accounts[1] });
      // let nftContract = token.address;
      //  initialBalance0 = await web3.eth.getBalance(accounts[0]);
      let items = await marketplace.fetchMarketItems({ from: accounts[0] });
      let item = items[0];
      ...
      assert.equal(item.seller, seller, "Item should be created successfully seller");
      assert.equal(item.price, price, "Item should be created successfully price");
      assert.equal(item.amount, itemPre.amount - amountSell, "Should have less items for sale after buying");
    ...
    it("cannot buy more items than for sale", async () => {
      await truffleAssert.reverts(marketplace.buyMarketItem(itemID, 100, { value: price, from: accounts[1] }));
    });

    it("cannot buy non-existing item", async () => {
      await truffleAssert.reverts(marketplace.buyMarketItem(123, amountSell, { value: price, from: accounts[1] }));
    });

    it("cannot buy if you are the seller", async () => {
      await truffleAssert.reverts(
        marketplace.buyMarketItem(itemID, amountSell, { value: price * amountSell, from: accounts[0] }),
      );
    });

    it("cannot buy if insuffient funds", async () => {
      await truffleAssert.reverts(marketplace.buyMarketItem(itemID, amountSell, { value: price, from: accounts[1] }));
    });

    it("can buy all items", async () => {
      let itemsPre = await marketplace.fetchMarketItems({ from: accounts[0] });
      let itemPre = itemsPre[0];
      await marketplace.buyMarketItem(itemID, itemPre.amount, { value: price * itemPre.amount, from: accounts[1] });
      let items = await marketplace.fetchMarketItems({ from: accounts[0] });
    ...
    it("cannot buy when sold out", async () => {
      await truffleAssert.reverts(marketplace.buyMarketItem(itemID, 1, { value: price, from: accounts[1] }));
    });
  });
});
