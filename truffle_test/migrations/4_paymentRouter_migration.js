const MarketplaceGetters = artifacts.require("MarketplaceGetters")
...
module.exports = async function (deployer, network, accounts) {
  ...
  await deployer.deploy(MarketplaceGetters, treasury, [seller], 300, 10000)
  ...
  let market = await MarketplaceGetters.deployed();
...
console.log("Running createMarketItem() for itemID 1:");
  await market.createMarketItem(tokenContract, seller, tokenID, sellAmount1, price, paymentContract, isPull, true, routeID, itemLimit1, false, {from: seller});
  ...
  console.log("itemID " + await itemID + " has " + await itemToken.balanceOf(market.address, tokenID) + " units available");
console.log("Running createMarketItem() for itemID 2:");
  await market.createMarketItem(tokenContract, seller, tokenID2, sellAmount2, price2, paymentContract, isPush, true, routeID, itemLimit2, false, {from: seller});
  itemID2 = await market.getLastItemID();
  console.log("itemID2: " + await itemID2);
  console.log("itemID " + itemID2 + " has " + await itemToken.balanceOf(market.address, tokenID2) + " units available");
